// ============================================================================
// sefaz-backend/cfop-catalogo.js  (PURO — testável · GERADO da tabela oficial)
// ----------------------------------------------------------------------------
// A DESCRIÇÃO OFICIAL DO CFOP — e, principalmente, DIZER quando um código NÃO
// CONSTA da redação em vigor, que é como o app inventa CFOP sem ninguém ver.
//
// ═══ FONTE ══════════════════════════════════════════════════════════════════
//
//   Anexo II do Convênio s/nº, de 15 de dezembro de 1970
//   REDAÇÃO DADA PELO AJUSTE SINIEF 03/24
//   (compilação da SEFAZ-SC, versão 2.0, 90 páginas — PDF mandado pelo Paulo
//    em 18/08; confaz.fazenda.gov.br é bloqueado pela rede deste ambiente)
//
// 619 códigos, extraídos do PDF por script — NENHUM digitado de memória.
//
// ═══ 🚨 A TRAVA QUE ESCOLHE A TABELA CERTA ══════════════════════════════════
//
// A PRIMEIRA tabela mandada (página `cfop_cvsn_70_nova` do CONFAZ) NÃO servia:
// ela é a redação do Ajuste SINIEF 16/20, que o próprio documento marca como
// "sem efeitos" e "revogado a partir de 01.06.22". A prova prática é que ela
// **elimina a família ST inteira** — sem 1401, 1403, 1406..1411.
//
// E o CFI escritura 1403 todo mês (56 notas no Resumo por CFOP da NOVA ERA
// 07/2026). Carregá-la faria o app dizer "não consta" para um código que ele
// mesmo produz, e alguém "corrigiria" a escrituração por uma tabela morta.
//
// **REGRA: tabela de CFOP sem a família ST (x401/x403/x407/x410/x411) é a
// redação do 16/20 e NÃO SERVE.** Travado em `cfopCatalogo.test.ts`.
//
// ═══ POR QUE ISTO EXISTE ════════════════════════════════════════════════════
//
...
// uso/consumo"*. Ele estava certo — o app citava a tabela oficial e conhecia
// DOIS códigos.
//
// E a descrição junto do número é o que faz o erro aparecer: numa nota da
// Kalunga a régua escreve "1101 — Compra para industrialização", e quem lê
// percebe na hora que material de escritório é 1556 ou 1551.
//
// ⚠️ ENTRADA NOVA SÓ ENTRA COPIADA DA TABELA — nunca escrita de memória.
// Descrição errada é pior que descrição nenhuma: faz escolher com CONFIANÇA o
// CFOP errado. Este arquivo é GERADO; para atualizar, regere do PDF novo.
// ============================================================================

/** A fonte — vai junto da descrição em toda tela que a mostrar. */
export const FONTE_CFOP = {
    titulo: 'CFOP — Anexo II do Convênio s/nº 15/12/1970 (redação do Ajuste SINIEF 03/24)',
    url: 'https://www.confaz.fazenda.gov.br/legislacao/ajustes/sinief/copy_of_cfop_cvsn_70_nova',
    redacao: 'Ajuste SINIEF 03/24',
};

/**
 * Descrições oficiais, extraídas do PDF da tabela em vigor.
 * Chave = 4 dígitos (sem o ponto). Valor = a descrição CURTA do código.
 */
export const CFOP_DESCRICOES = {
    ...
    '1120': 'Compra para industrialização ou produção rural, em venda à ordem, já recebida do vendedor remetente',
    ...
    '1131': 'Entrada de mercadoria, com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    ...
    '1214': 'Devolução referente à fixação de preço de produção do estabelecimento produtor em ato cooperativo',
    '1215': 'Devolução de fornecimento de produção do estabelecimento em ato cooperativo',
    '1216': 'Devolução de fornecimento de mercadoria adquirida ou recebida de terceiros em ato cooperativo',
    ...
    '1504': 'Entrada decorrente de devolução, inclusive simbólica, de mercadoria remetida com fim específico de exportação, adquirida ou recebida de terceiros',
    '1505': 'Entrada decorrente de devolução, inclusive simbólica, de mercadorias remetidas para formação de lote de exportação, de produtos industrializados ou produzidos pelo próprio estabelecimento',
    '1506': 'Entrada decorrente de devolução, inclusive simbólica, de mercadorias adquiridas ou recebidas de terceiros, remetidas para formação de lote de exportação',
    ...
    '1660': 'Devolução de venda de combustíveis ou lubrificantes destinados à industrialização subsequente',
    '1661': 'Devolução de venda de combustíveis ou lubrificantes destinados à comercialização',
    '1662': 'Devolução de venda de combustíveis ou lubrificantes destinados a consumidor ou usuário final',
    ...
    '1905': 'Entrada de mercadoria recebida para depósito em depósito fechado, armazém geral ou outro estabelecimentoda mesma empresa ou de terceiro',
    '1906': 'Retorno de mercadoria remetida para depósito fechado, armazém geral ou outro estabelecimento, da mesma empresa ou de terceiro',
    '1907': 'Retorno simbólico de mercadoria remetida para depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    ...
    '1934': 'Entrada simbólica de mercadoria recebida para depósito em depósito fechado ou armazém geral',
    ...
    '2120': 'Compra para industrialização ou produção rural, em venda à ordem, já recebida do vendedor remetente',
    ...
    '2131': 'Entrada de mercadoria, com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    ...
    '2214': 'Devolução referente à fixação de preço de produção do estabelecimento produtor, em ato cooperativo',
    '2215': 'Devolução de fornecimento de produção do estabelecimento em ato cooperativo',
    '2216': 'Devolução de fornecimento de mercadoria adquirida ou recebida de terceiros em ato cooperativo',
    ...
    '2504': 'Entrada decorrente de devolução, inclusive simbólica, de mercadoria remetida com fim específico de exportação, adquirida ou recebida de terceiros',
    '2505': 'Entrada decorrente de devolução, inclusive simbólica, de mercadorias remetidas para formação de lote de exportação, de produtos industrializados ou produzidos pelo próprio estabelecimento',
    '2506': 'Entrada decorrente de devolução, inclusive simbólica, de mercadorias adquiridas ou recebidas de terceiros, remetidas para formação de lote de exportação',
    ...
    '2662': 'Devolução de venda de combustíveis ou lubrificantes destinados a consumidor ou usuário final',
    ...
    '2905': 'Entrada de mercadoria recebida para depósito em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    '2906': 'Retorno de mercadoria remetida para depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    '2907': 'Retorno simbólico de mercadoria remetida para depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    ...
    '2934': 'Entrada simbólica de mercadoria recebida para depósito em depósito fechado ou armazém geral',
    ...
    '3552': 'Entrada de produto destinado para uso ou consumo de bordo, em embarcações ou aeronaves exclusivamente em tráfego internacional com destino ao exterior',
    ...
    '3667': 'Entrada de combustíveis ou lubrificantes para consumo final, em embarcações ou aeronaves exclusivamente em tráfego internacional com destino ao exterior',
    ...
    '5118': 'Venda de produção do estabelecimento industrial ou rural, entregue ao destinatário por conta e ordem do adquirente originário, em venda à ordem',
    ...
    '5129': 'Venda de mercadoria industrializada e de insumo importado sob o amparo do Regime Aduaneiro Especial de Entreposto Industrial sob Controle Informatizado do Sistema Público de Escrituração Digital (Recof-Sped)',
    '5131': 'Remessa de produção do estabelecimento, com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    '5132': 'Fixação de preço de produção do estabelecimento, inclusive quando remetidas anteriormente com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    ...
    '5155': 'Transferência de produção do estabelecimento para depósito em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    '5156': 'Transferência de mercadoria adquirida ou recebida de terceiros para depósito em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    ...
    '5213': 'Devolução de entrada, inclusive simbólica, de mercadoria com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    '5214': 'Devolução referente à fixação de preço de produção do estabelecimento produtor, inclusive quando remetidas anteriormente com previsão de posterior ajuste ou fixação de preço, em ato cooperativo, para comercialização',
    '5215': 'Devolução referente à fixação de preço de produção do estabelecimento produtor, inclusive quando remetidas anteriormente com previsão de posterior ajuste ou fixação de preço, em ato cooperativo, para industrialização',
    '5216': 'Devolução de entrada decorrente do fornecimento de produto ou mercadoria em ato cooperativo',
    ...
    '5413': 'Devolução de mercadoria destinada para uso ou consumo, em operação com mercadoria sujeita ao regime de substituição tributária',
    ...
    '5651': 'Venda de combustíveis ou lubrificantes de produção do estabelecimento destinados à industrialização subsequente',
    '5652': 'Venda de combustíveis ou lubrificantes de produção do estabelecimento destinados à comercialização',
    '5653': 'Venda de combustíveis ou lubrificantes de produção do estabelecimento destinados a consumidor ou usuário final',
    '5654': 'Venda de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros destinados à industrialização subsequente',
    '5655': 'Venda de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros destinados à comercialização',
    '5656': 'Venda de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros destinados a consumidor ou usuário final',
    '5657': 'Remessa de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros para venda fora do estabelecimento',
    ...
    '5660': 'Devolução de compra de combustíveis ou lubrificantes adquiridos para industrialização subsequente',
    '5661': 'Devolução de compra de combustíveis ou lubrificantes adquiridos para comercialização',
    '5662': 'Devolução de compra de combustíveis ou lubrificantes adquiridos por consumidor ou usuário final',
    ...
    '5666': 'Remessa por conta e ordem de terceiros de combustíveis ou lubrificantes recebidos para armazenagem',
    '5667': 'Venda de combustíveis ou lubrificantes a consumidor ou usuário final estabelecido em outra unidade da Federação',
    ...
    '5905': 'Remessa para depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    '5906': 'Retorno de mercadoria depositada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    '5907': 'Retorno simbólico de mercadoria depositada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    ...
    '5920': 'Remessa de embalagens, bombonas, vasilhames, sacarias, pallets, containers ou assemelhados',
    '5921': 'Devolução de embalagens, bombonas, vasilhames, sacarias, pallets, containers ou assemelhados',
    ...
    '5927': 'Lançamento efetuado a título de baixa de estoque decorrente de perda, roubo, deterioração',
    '5928': 'Lançamento efetuado a título de baixa de estoque decorrente do encerramento da atividade do estabelecimento da empresa ou transferência por venda do fundo de comércio',
    '5929': 'Lançamento efetuado em decorrência de emissão de documento fiscal relativo à operação ou prestação também acobertada por documento fiscal do varejo',
    ...
    '6105': 'Venda de produção do estabelecimento armazenada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    '6106': 'Venda de mercadoria adquirida ou recebida de terceiros armazenada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    ...
    '6118': 'Venda de produção do estabelecimento industrial ou rural, entregue ao destinatário por conta e ordem do adquirente originário, em venda à ordem',
    ...
    '6129': 'Venda de mercadoria industrializada e de insumo importado sob o amparo do Regime Aduaneiro Especial de Entreposto Industrial sob Controle Informatizado do Sistema Público de Escrituração Digital (Recof-Sped)',
    '6131': 'Remessa de produção do estabelecimento, com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    '6132': 'Fixação de preço de produção do estabelecimento, inclusive quando remetidas anteriormente com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    ...
    '6155': 'Transferência de produção do estabelecimento armazenada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    '6156': 'Transferência de mercadoria adquirida ou recebida de terceiros armazenada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    ...
    '6213': 'Devolução de entrada, inclusive simbólica, de mercadoria, com previsão de posterior ajuste ou fixação de preço, em ato cooperativo',
    '6214': 'Devolução referente à fixação de preço de produção do estabelecimento produtor, inclusive quando remetidas anteriormente com previsão de posterior ajuste ou fixação de preço, em ato cooperativo, para comercialização',
    '6215': 'Devolução referente à fixação de preço de produção do estabelecimento produtor, inclusive quando remetidas anteriormente com previsão de posterior ajuste ou fixação de preço, em ato cooperativo, para industrialização',
    '6216': 'Devolução de entrada decorrente do fornecimento de produto ou mercadoria em ato cooperativo',
    ...
    '6413': 'Devolução de mercadoria destinada para uso ou consumo, em operação com mercadoria sujeita ao regime de substituição tributária',
    ...
    '6651': 'Venda de combustíveis ou lubrificantes de produção do estabelecimento destinados à industrialização subsequente',
    '6652': 'Venda de combustíveis ou lubrificantes de produção do estabelecimento destinados à comercialização',
    '6653': 'Venda de combustíveis ou lubrificantes de produção do estabelecimento destinados a consumidor ou usuário final',
    '6654': 'Venda de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros destinados à industrialização subsequente',
    '6655': 'Venda de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros destinados à comercialização',
    '6656': 'Venda de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros destinados a consumidor ou usuário final',
    '6657': 'Remessa de combustíveis ou lubrificantes adquiridos ou recebidos de terceiros para venda fora do estabelecimento',
    ...
    '6660': 'Devolução de compra de combustíveis ou lubrificantes adquiridos para industrialização subsequente',
    '6661': 'Devolução de compra de combustíveis ou lubrificantes adquiridos para comercialização',
    '6662': 'Devolução de compra de combustíveis ou lubrificantes adquiridos por consumidor ou usuário final',
    ...
    '6666': 'Remessa por conta e ordem de terceiros de combustíveis ou lubrificantes recebidos para armazenagem',
    '6667': 'Venda de combustíveis ou lubrificantes a consumidor ou usuário final estabelecido em outra unidade da Federação diferente da que ocorrer o consumo',
    ...
    '6905': 'Remessa para depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    '6906': 'Retorno de mercadoria depositada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    '6907': 'Retorno simbólico de mercadoria depositada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro',
    ...
    '6920': 'Remessa de embalagens, bombonas, vasilhames, sacarias, pallets, containers ou assemelhados',
    '6921': 'Devolução de embalagens, bombonas, vasilhames, sacarias, pallets, containers ou assemelhados',
    ...
    '6929': 'Lançamento efetuado em decorrência de emissão de documento fiscal relativo à operação ou prestação também acobertada por documento fiscal do varejo',
    ...
    '7105': 'Venda de produção do estabelecimento armazenada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    '7106': 'Venda de mercadoria adquirida ou recebida de terceiros armazenada em depósito fechado, armazém geral ou outro estabelecimento da mesma empresa ou de terceiro, que não deva por ele transitar',
    ...
    '7552': 'Saída de produto destinado para uso ou consumo de bordo, em embarcações ou aeronaves exclusivamente em tráfego internacional com destino ao exterior',
    ...
    '7667': 'Venda de combustíveis ou lubrificantes a consumidor ou usuário final',
...
/**
 * A descrição oficial do CFOP, ou **null** quando o código NÃO CONSTA da
 * redação em vigor.
 *
 * Null é de propósito: com o catálogo COMPLETO, "não achei" deixa de significar
 * "falta cadastrar" e passa a significar **"este CFOP não existe"** — que é o
 * alarme que revela o app inventando código (foi assim que apareceram o 1405 e
 * o 1655).
 */
...
export function cfopExiste(codigo) {
    return descricaoCfop(codigo) !== null;
}
...
/**
 * A frase para a tela: a descrição quando o código existe, e a AUSÊNCIA nomeada
 * quando não — com a palavra certa, que agora é "não consta na tabela", não
 * "falta cadastrar".
 */
export function textoDoCfop(codigo) {
    ...
    return {
        temDescricao: false,
        texto: c + ' NAO CONSTA na tabela em vigor (' + FONTE_CFOP.redacao + ') — confira antes de escriturar.',
        fonte: FONTE_CFOP,
    };
}

/**
 * Os CFOPs de uma lista que NÃO constam da tabela — a conferência que o Paulo
 * pediu, feita contra a NORMA em vez de contra outro sistema.
 */
export function cfopsInexistentes(codigos) {
    const fora = [];
    for (const c of (codigos || [])) {
        const d = so(c);
        if (d.length === 4 && !CFOP_DESCRICOES[d] && !fora.includes(d)) fora.push(d);
    }
    return fora.sort();
}
