...
| Outros canais — Instagram | **[?]** | **DMs no MESMO inbox (construído 22/08)**: a sonda ✅ conta-encontrada [Paulo, print — Página "SP Assessoria Contábil" · @spassessoriacontabil] destravou a fase 2 no mesmo dia. Conversa `ig_{IGSID}` com selo 📷 nas MESMAS coleções — fila, assumir, transferir, nota e relatório funcionam sem mudar nada; **o bot NÃO roda nas DMs** (menu numérico é contrato do WhatsApp), a DM cai na triagem da Recepção e gente conduz. Resposta por TEXTO na janela da Meta (fora dela não há template no IG — espera-se o cliente escrever). Recebimento nasce **DESLIGADO**: liga no 📡 da ⚙️ → 📷 | 🟡 construído; 📡 clicado ✅ (22/08) e a 1ª DM real NÃO chegou — o print do painel [Paulo] revelou a causa: o caso de uso é **"API do Instagram com login do Instagram"**, que tem app do Instagram PRÓPRIO (API_Oficial-IG · 1586692779912611), chave secreta própria e o webhook configurado NA TELA DO CASO DE USO (seção 3), não no webhook geral. O app já aceita as duas assinaturas e envia pelo `graph.instagram.com` quando houver token da conta. **Configurado pelo Paulo em 22/08** (seção 3 + secrets `instagram-app-secret`/`instagram-access-token` → envs `INSTAGRAM_APP_SECRET`/`INSTAGRAM_ACCESS_TOKEN`; "pode rodar" = deploy de ativação disparado; secrets criados e referenciados no serviço, 2º deploy de ativação em 22/08 — o 🔑 da ⚙️ → 📷 é quem confirma as envs NO AR). **Falta a PROVA**: DM nova de teste chegando com o selo 📷 e a resposta voltando — o contador da ⚙️ → 📷 é quem diz. Anexo/áudio de SAÍDA no IG segue fase futura |
...
7. ✅ ~~**Outros canais**~~ — **Instagram CONSTRUÍDO 22/08** (sonda ✅ →
   DMs no inbox no mesmo dia; falta o clique no 📡 e uma DM real — ver a
   linha do §5). Wix ainda é pergunta aberta: **qual recurso do Wix** — o
