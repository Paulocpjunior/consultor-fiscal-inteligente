// backend/email-layout.js  (PURO, exceto anexoLogo — testável) — PORTE DO CFI PARA O CCI (24/09)
//
// Copiado de sefaz-backend/email-layout.js do consultor-fiscal-inteligente.
// Diferença ÚNICA de identidade: `MARCA.departamento` = 'Departamento Contábil'.
// Regra de porte: os repos não compartilham código — mudança de layout precisa
// ser feita nas DUAS cascas (esta e a do CFI).
//
...
export function montarEmailGuia({ tipo, empresaNome, competencia, mensagem, temPdf, vencimento, geradoEm }) {
    ...
    return montarLayoutEmail({
        ...
        selo,
        conteudoHtml,
        assinatura: `Enviado pelo Consultor Contábil Inteligente em ${escaparHtml(geradoEm || new Date().toLocaleString('pt-BR', { timeZone: 'America/Sao_Paulo' }))}.`,
...
export function anexoLogo() {
    if (logoEmailCache === undefined) {
        try {
            const caminho = path.join(process.cwd(), 'backend', 'assets', 'sp-logo-email.png');
            logoEmailCache = {
                name: 'sp-logo.png',
