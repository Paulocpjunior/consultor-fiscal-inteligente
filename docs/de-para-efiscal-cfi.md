...
| **Bloco E — IPI (E500/E510/E520)** | ✔ | `sped-bloco-e-st.js`/`buildE500E520` (período+apuração) + `sped-bloco-ipi-e510.js` (11 testes) para a consolidação por CFOP+CST_IPI | 🟡 **E510 no ar 11/08** — era o `🔴` que travava indústria com IPI. `VL_CONT_IPI` = Σ vProd (NÃO inclui IPI) foi **PROVADO contra arquivo aceito no PVA** (SPED jun/2026 do e-Fiscal): bateu campo a campo com o `VL_OPR` do C190 em 1101/5101/5122/6101, e a Σ do `VL_IPI` de saída = `VL_DEB` do E520 (11.742,05 cravado). O E510 é o C190 re-agregado por CFOP+CST_IPI, então reaproveita `convertCfopParaEntrada`. Captura do `cstIpi`/`vBcIpi` no importer (item com IPI sem CST capturado fica FORA e ACENDE aviso — CST não se chuta). **Falta:** backfill dos XMLs de jun/jul já capturados (o doc guarda `xmlHash`, não o XML cru — re-capturar os poucos clientes de IPI é o caminho) e a validação final no PVA. **E510 não gera CST do IPI inventado** |
