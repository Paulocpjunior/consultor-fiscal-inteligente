...
| SPED Contribuições (PIS/COFINS) | ✔ | card SPED Fiscal → aba Contribuições | 🟡 **REBAIXADO 18/08** — MANTOAN 07/2026 zerou no PVA depois de 3 rodadas (COD_ITEM/IND_ORIG_CRED do A170), mas a HS PROJETOS (retida de PIS/COFINS/CSLL) revelou que **`buildBlocoF` é um STUB**: sempre sai `F001=1` (bloco sem dados), NUNCA gera F600 (Contribuição Retida na Fonte) — pra nenhuma empresa. O dado de retenção já existe no app (Memória de Apuração mostra PIS/COFINS/CSLL retidos calculados certinhos); falta o gerador escrever. Bloqueado por evidência: F600 tem campo `IND_NAT_RET` de tabela oficial, e a régua da casa proíbe chutar código — aguardando um arquivo antigo do E-Fiscal com F600 real ou print da tela do PVA |
