...
| **Pré-validação ("PVA de bolso")** | — | `sped-prevalidacao.js` — header `X-SPED-Prevalidacao` + warnings da geração | ✅ **20/08** — só existe no CFI. Nasceu da queixa do gargalo (*"evitando o vai e vem o dia todo"*): as recusas apareciam **um round-trip do PVA por vez**, e cada volta custava um dia. As 14 regras rodam sobre as LINHAS do arquivo gerado, cada uma com a recusa LITERAL do PVA (ou a citação do Guia 3.2.3) como fonte, com cliente e data. **Regra que fica: recusa nova entra aqui no MESMO PR** — recusa que só conserta a instância volta no mês seguinte com outro CNPJ |
