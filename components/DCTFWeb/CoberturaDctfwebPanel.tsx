/**
 * Painel de cobertura DCTFWeb — heatmap por empresa Lucro × competência.
 * Verde = ATIVA, amarelo = EM_ANDAMENTO, vermelho = não transmitida.
 */
import React, { useEffect, useMemo, useState } from 'react';
import { getCoberturaDctfweb, type CoberturaDctfwebResposta, type MesCoberturaDctfweb } from '../../services/dctfwebCoberturaService';

interface Props { onShowToast?: (msg: string) => void; }

const CoberturaDctfwebPanel: React.FC<Props> = ({ onShowToast: _onShowToast }) => {
    const [meses, setMeses] = useState(6);
    const [data, setData] = useState<CoberturaDctfwebResposta | null>(null);
    const [loading, setLoading] = useState(true);
    const [erro, setErro] = useState<string | null>(null);
    const [busca, setBusca] = useState('');
    const [filtro, setFiltro] = useState<'todas' | 'com-gap' | 'em-andamento'>('com-gap');

    const carregar = async () => {
        setLoading(true); setErro(null);
        try { setData(await getCoberturaDctfweb(meses)); }
    ...
    const lista = useMemo(() => {
        ...
        return data.empresas.filter((e) => {
            ...
            if (filtro === 'em-andamento' && e.emAndamento === 0) return false;
            if (!q) return true;
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="p-6 rounded-xl" style={{ background: 'linear-gradient(135deg, var(--accent-soft), var(--bg-elevated))', border: '1px solid var(--border-default)' }}>
                <h2 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Cobertura DCTFWeb</h2>
                <p className="text-sm mt-1" style={{ color: 'var(--text-secondary)' }}>
                    Para cada empresa Lucro Presumido/Real, mostra quais competências
                    dos últimos meses <b>não têm DCTFWeb transmitida (situação ATIVA)</b>.
                    Mesmo risco do PGDAS-D no Simples: a Receita autua automaticamente.
                    O mês atual não entra (ainda não venceu).
                </p>

                {data && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
                        <Kpi label="Empresas Lucro" value={String(data.totalEmpresas)} />
                        ...
                        <Kpi label="Em andamento (não transmitida)" value={String(data.totalEmAndamento)} accent={data.totalEmAndamento > 0 ? 'warning' : 'success'} />
                    </div>
                )}
            </div>
            ...
            <div className="p-4 rounded-xl flex flex-wrap items-center gap-3" style={card}>
                ...
                <div className="flex gap-1 text-xs">
                    {(['com-gap', 'em-andamento', 'todas'] as const).map(f => (
                        <button key={f} onClick={() => setFiltro(f)}
                            ...
                            {f === 'com-gap' ? 'Com gap' : f === 'em-andamento' ? 'Em andamento' : 'Todas'}
                        </button>
                    ))}
                </div>
            ...
            {data && (
                <div className="flex gap-4 text-xs px-2" style={{ color: 'var(--text-muted)' }}>
                    ...
                    <span className="flex items-center gap-1"><Cell mes={{ competencia: '', transmitido: true, situacao: 'EM_ANDAMENTO' }} mini /> Em andamento</span>
                    <span className="flex items-center gap-1"><Cell mes={{ competencia: '', transmitido: false }} mini /> NÃO transmitida</span>
                </div>
            )}
                    ...
                    <tbody>
                        ...
                        {lista.map((emp) => (
                            <tr key={emp.cnpj} style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                                ...
                                {emp.meses.map((m) => (
                                    <td key={m.competencia} className="py-1 px-0.5 text-center"><Cell mes={m} /></td>
                                ))}
                                <td className="py-2 px-2 text-center">
                                    <span className="text-xs font-bold" style={{ color: emp.gaps > 0 ? 'var(--danger)' : 'var(--success)' }}>{emp.gaps}</span>
                                </td>
                            </tr>
                        ))}
                    </tbody>
...
function Cell({ mes, mini }: { mes: MesCoberturaDctfweb; mini?: boolean }) {
    let bg = 'var(--danger)', titulo = `${mes.competencia} — NÃO transmitida`;
    if (mes.transmitido) {
        if (mes.situacao === 'ATIVA') { bg = 'var(--success)'; titulo = `${mes.competencia} — transmitida (ATIVA)`; }
        else { bg = 'var(--warning)'; titulo = `${mes.competencia} — em andamento (não transmitida ainda)`; }
    }
    const sz = mini ? 12 : 22;
...
export default CoberturaDctfwebPanel;
