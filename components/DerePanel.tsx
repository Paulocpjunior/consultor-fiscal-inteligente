 ...
 * eventos. O leiaute não pôde ser lido nesta rede e o insumo (plano de contas,
 * balancete) é contábil. Prometer geração aqui seria a promessa que a tela não
 * cumpre (a lição do ✕ de 14/08).
 *
...
const ROTULO_SITUACAO: Record<string, string> = {
    ...
    'regime-nao-confirmado': 'regime específico marcado — alcance não confirmado',
                ...
                <div className="mt-3 space-y-3">
                    <div className="rounded-lg border border-slate-200 dark:border-slate-700 p-2">
                        ...
                        <p className="mt-1 text-slate-600 dark:text-slate-300">
                            ...
                            {r.resumo.regimeNaoConfirmado} com regime não confirmado · {r.resumo.naoSeAplica} marcada(s) "não se aplica" ·{' '}
                    ...
                    <div className="rounded-lg border-l-4 border-amber-500 bg-amber-50 dark:bg-amber-900/20 p-2 text-amber-800 dark:text-amber-300">
                        ...
                        oficiais não puderam ser lidos nesta rede, e o insumo dos eventos (plano de contas
                        comentado, balancete mensal) é contábil. A entrega é por fora; o mês do cliente passa a
                        cobrar a obrigação, e ela se registra em Vencimentos como entregue fora do app.
                    </div>

                    <div>
                        <h5 className="font-bold text-slate-700 dark:text-slate-200">Obrigadas ({r.obrigadas.length})</h5>
                        {r.obrigadas.length
                    ...
                    {!!r.regimeNaoConfirmado.length && (
                        <div>
                            <h5 className="font-bold text-slate-700 dark:text-slate-200">Regime específico marcado, alcance da DeRE não confirmado ({r.regimeNaoConfirmado.length})</h5>
                            <Tabela linhas={r.regimeNaoConfirmado} />
                        </div>
                    )}
                    ...
                    <details>
                        ...
                        {r.vigente && (
                            <div className="mt-2">
                                ...
                                <ul className="ml-4 list-disc">{r.eventos.mensais.map(e => <li key={e.codigo}><code>{e.codigo}</code> {e.nome}{e.nota ? <span className="text-slate-400"> — {e.nota}</span> : null}</li>)}</ul>
                            </div>
                        )}
                    </details>
