...
export interface EntradaCalculo { receitaBrutaMensal:number; receitaBrutaAcumulada12:number; folhaPagamentoMensal:number; custosMercadorias:number; despesasDedutiveis:number; creditosPisCofins:number; atividade:AtividadeTipo; issAliquota:number; temImportacao:boolean; }
export interface ResultadoRegime { regime:RegimeTipo; label:string; impostoMensal:number; aliquotaEfetiva:number; detalhamento:Record<string,number>; observacoes:string[]; recomendado?:boolean; }
export interface ResultadoAnalise { resultados:ResultadoRegime[]; melhorOpcao:RegimeTipo; economiaAnual:number; percentualEconomia:number; alertas:string[]; }
