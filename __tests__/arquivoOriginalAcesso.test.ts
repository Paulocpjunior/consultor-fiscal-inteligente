...
jest.mock('firebase-admin', () => ({ __esModule: true, default: {
    apps: [{}], firestore: () => ({ collection: (col: string) => ({
        doc: (id: string) => ({ get: async () => ({ exists: mockDocs.has(`${col}/${id}`), data: () => mockDocs.get(`${col}/${id}`) }) }),
        where: () => ({ limit: () => ({ get: async () => ({ empty: false, docs: [{ data: () => mockDocs.get('documentos_fiscais/n') }] }) }) }),
    }) }),
} }));
jest.mock('@google-cloud/storage', () => ({ Storage: jest.fn(() => ({ bucket: () => ({ file: () => ({ download: mockDownload, save: mockSave, exists: async () => [true] }) }) })) }));
...
const xmlHandler = router.stack.find((layer: any) => layer.route?.path === '/xml-bruto').route.stack.at(-1).handle;
...
test('rota antiga de XML tambem nega caminho forjado de outra empresa', async () => {
    mockDocs.set('documentos_fiscais/n', { empresaId: 'a', storagePath: 'xmls/b/segredo.xml' });
    const res = response();
    await xmlHandler({ query: { chave: '1'.repeat(44) }, user: { uid: 'membro' } }, res);
    expect(res.status).toHaveBeenCalledWith(404);
    expect(mockDownload).not.toHaveBeenCalled();
});
test('rota antiga preserva download da empresa autorizada', async () => {
    const res = response();
    await xmlHandler({ query: { chave: '1'.repeat(44) }, user: { uid: 'membro' } }, res);
    expect(mockDownload).toHaveBeenCalledTimes(1);
    expect(res.setHeader).toHaveBeenCalledWith('Cache-Control', 'private, no-store');
