 ...
 */
import {
    contraparteNormalizada, ehNotaPropriaDeEntrada, escrituraveisNoLivroDeEntradas,
...
describe('o livro da VINCENZO deixa de dobrar', () => {
    it('8 notas viram 4 — e o total cai de 37.800 para 18.900', () => {
        const r = escrituraveisNoLivroDeEntradas(DOCS, montar, valorDe, EMPRESA);
...
describe('o que NÃO pode sumir do livro', () => {
    it('NF-e de produtor SEM par fica — muitos clientes escrituram ela direto', () => {
        const r = escrituraveisNoLivroDeEntradas(
            [doProdutor('95', '16', 5200), doProdutor('96', '20', 5000)], montar, valorDe, EMPRESA,
        );
    ...
    it('valor diferente NÃO pareia — sem prova, escritura as duas', () => {
        const r = escrituraveisNoLivroDeEntradas(
            [doProdutor('95', '16', 5200), propria('16', '18', 4000)], montar, valorDe, EMPRESA,
        );
        expect(r.linhas).toHaveLength(2);
        expect(r.excluidas).toEqual([]);
    });
    ...
    it('sem contraparte no documento, nada é excluído', () => {
        ...
        const r = escrituraveisNoLivroDeEntradas(
            [semParte, { ...propria('16', '18', 5200), destinatario: {} }], montar, valorDe, EMPRESA,
        );
        expect(r.linhas).toHaveLength(2);
        expect(r.excluidas).toEqual([]);
    });
    ...
    it('nota de FORNECEDOR normal (PJ) não é tocada', () => {
        ...
        const r = escrituraveisNoLivroDeEntradas([fornecedor, propria('16', '18', 5200)], montar, valorDe, EMPRESA);
        expect(r.linhas).toHaveLength(2);
        expect(r.excluidas).toEqual([]);
    });
});
...
describe('🚨 a forma REAL do banco (pré-backfill) é reconhecida', () => {
    ...
    it('e a dedup roda: 8 notas gravadas como no banco viram 4', () => {
        ...
        const r = escrituraveisNoLivroDeEntradas(docs, montar, (d: any) => d.valorTotal, EMPRESA);
        expect(r.linhas).toHaveLength(4);
